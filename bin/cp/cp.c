 ...
 *   cp [-r] <sorgente…>   <directory>
 *   cp [-r] <jolly>       <directory>
 *   cp -h
 *
 * Opzioni
 ...
 *   -h   aiuto ed esempi
 *
 * NON SOVRASCRIVE file esistenti. Chi vuole sostituire cancella prima.
 * =============================================================================
 *
 * Logica jolly
 * ─────────────
 ...
 *
 * ⚠️ '.' e '..' vengono SEMPRE SALTATI — sia in readdir per la copia
 ...
 * Non sovrascrive: se dst esiste, errore.
 * Gestisce la write parziale: torna finché tutti i byte non sono scritti. */
...
static int copia_come(const char *src, const char *dst)
{
    ...
    if (S_ISDIR(st.st_mode)) {
        ...
        return copia_dir(src, dst) > 0 ? -1 : 0;
    }
...
{
    printf("uso:\n");
    printf("  cp [-r] <sorgente>  <destinazione>\n");
    printf("  cp [-r] <sorgente>  <directory>\n");
    printf("  cp [-r] <sorgente1> <sorgente2> ... <directory>\n");
    printf("  cp [-r] <jolly>     <directory>\n");
    printf("  cp -h\n");
    printf("\n");
    printf("opzioni:\n");
    ...
    printf("\n");
    printf("note:\n");
    printf("  Non sovrascrive file esistenti.\n");
