 ...
 *
 * -----------------------------------------------------------------------------
 * ⚠️ TELNET MANDA TUTTO IN CHIARO, PASSWORD COMPRESA
 *
 ...
 *
 * ⚠️ LA SCADENZA SERVE ANCHE QUANDO NON SCADE NIENTE. La modalita' raw
 ...
 *
 * ⚠️ LA REGOLA E' «RIFIUTA TUTTO QUELLO CHE NON SAI FARE», e va detta
 ...
 * aspetta.
 *
 * ⚠️ E NON SI RISPONDE MAI A UNA RISPOSTA. Se a un DO si replicasse
...
static int manda(const unsigned char *dati, unsigned int len)
{
    ...
    /* ⚠️ SI ASPETTA L'ESITO, e non e' pignoleria: lo stack risponde a
 ...
 *
 * ⚠️ LO STATO E' STATICO E NON LOCALE. Una sequenza IAC puo' essere
...
static void in_arrivo(const unsigned char *d, unsigned int len)
{
    ...
    for (i = 0; i < len; i++) {
        ...
        case 2:
            /* ⚠️ SI SVUOTA IL TESTO PRIMA DI RISPONDERE. La risposta e'
...
static int tasto_in_byte(unsigned int ev, unsigned char *out)
{
    ...
    switch (k) {
    /* ⚠️ INVIO E' CR LF, non un solo LF. Il terminale virtuale di telnet
    ...
    /* ⚠️ IL BACKSPACE SI MANDA COME 0x7F, non come 0x08 che e' quello
...
static void uso(void)
{
    ...
    printf("⚠️ Telnet manda tutto in chiaro, password compresa: non e' un\n");
...
int main(int argc, char **argv)
{
    ...
    printf("⚠️ Quello che scrivi viaggia in chiaro.\n\n");
