 ...
 *
 * -----------------------------------------------------------------------------
 * ⚠️ SOLO MODO PASSIVO (PASV), E NON E' UNA SEMPLIFICAZIONE
 *
 ...
 *
 * -----------------------------------------------------------------------------
 * ⚠️ FTP MANDA LA PASSWORD IN CHIARO
 *
 ...
 *
 * -----------------------------------------------------------------------------
 * ⚠️ LE RISPOSTE FTP POSSONO ESSERE SU PIU' RIGHE
 *
...
static int scrivi(int id, const unsigned char *src, unsigned int n)
{
    ...
    while (fatti < n) {
        ...
        /* ⚠️ LO STACK PUO' ACCETTARNE MENO DI QUANTI GLIENE OFFRIAMO
...
static int risposta(int mostra)
{
    ...
    for (;;) {
        ...
        if (primo) {
            ...
            primo = 0;
            /* ⚠️ IL TRATTINO DICE CHE CONTINUA. Senza questo controllo la
 ...
 *
 * ⚠️ SI CERCANO I NUMERI, NON SI SEGUE UN FORMATO. Il testo fra il codice
 ...
 *
 * ⚠️ SI USA L'INDIRIZZO DEL SERVER DI CONTROLLO, NON QUELLO ANNUNCIATO
...
/* ⚠️ UN CODICE 4xx O 5xx E' UN RIFIUTO, e va trattato come tale. La prima
