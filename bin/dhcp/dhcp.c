 ...
 *
 * -----------------------------------------------------------------------------
 * ⚠️ PERCHE' UN PROGRAMMA E NON UN PEZZO DELLO STACK
 *
 ...
 *
 * -----------------------------------------------------------------------------
 * ⚠️ SI PARTE AZZERANDO L'INDIRIZZO, E NON E' UN EFFETTO COLLATERALE
 *
 ...
 *
 * -----------------------------------------------------------------------------
 * ⚠️ IL BIT DI BROADCAST NELLA RICHIESTA
 *
                            ...
                            const unsigned char *server)
{
    ...
    /* ⚠️ SI RIEMPIE FINO A 300 BYTE. Certi server (e certi apparati di
                            ...
                            unsigned char cercata, const unsigned char **val)
{
    ...
    while (o < len) {
        ...
        l = p[o + 1];
        /* ⚠️ Una lunghezza che esce dal pacchetto e' un pacchetto
...
/* Uno scambio: manda, aspetta una risposta del tipo giusto.
 *
 * ⚠️ SI PRENOTA LA RICEZIONE PRIMA DI MANDARE, e l'ordine non e' un
 * dettaglio di stile.
 *
 ...
 *
 * ⚠️ E SI ACCETTANO I DUE MESSAGGI IN QUALUNQUE ORDINE. Dopo l'invio
                   ...
                   unsigned char *risposta, unsigned int *lunghezza)
{
    ...
    for (tentativo = 0; tentativo < TENTATIVI; tentativo++) {
        ...
        while ((int)(uptime_ms() - scadenza) < 0) {
            ...
            /* ⚠️ SI CONTROLLA xid E chaddr. Su una rete con piu' client che
...
int main(int argc, char **argv)
{
    ...
    /* ⚠️ SENZA MASCHERA NON SI PUO' INSTRADARE NIENTE, e certi server non
    ...
    printf("      ⚠️  la concessione NON viene rinnovata: quando scade,\n");
