 ...
 *
 * ⚠️ QUESTA FORMA E' SCELTA PER UN MOTIVO CHE OGGI NON SI VEDE. Il modo
 ...
 *
 * ⚠️ `riconosce` NON DEVE FIDARSI DI NIENTE. Gli si passa un volume che
 ...
 * MOTORE ext2
 *
 * ⚠️ COSA CONTROLLA UN CONTROLLORE ext2, E PERCHE' NON SONO LE STESSE COSE
 ...
 *      realta'.
 *
 * ⚠️ SI RIFIUTA DI RIPARARE UN VOLUME CON `incompat` SCONOSCIUTE. Un bit
...
/* ⚠️ QUANTE VOLTE NON SI E' POTUTO SAPERE. Ogni lettura fallita, ogni
 ...
 * quelle scritte sul volume. ⚠️ Sono la ragione per cui il programma alloca:
...
/* ⚠️ NIENTE BUFFER GLOBALE PER I BLOCCHI DI DIRECTORY. La prima stesura ne
...
static int e2_apri(void)
{
    ...
    /* ⚠️ I descrittori stanno nel blocco SUBITO DOPO il superblocco, e
    ...
    if (e2_incompat & ~(unsigned int)E2_INCOMPAT_NOTI) {
        ...
        problemi++;
        e2_incerto++;
        ripara = 0;          /* ⚠️ si disarma la riparazione, di proposito */
    }
...
/* =============================================================================
 * ⚠️ LE COPIE DI RISERVA DEL SUPERBLOCCO NON SONO DI NESSUN FILE
 *
 ...
 *
 * ⚠️ CON `sparse_super` LE COPIE STANNO SOLO nei gruppi 0 e 1 e in quelli
 ...
 *
 * ⚠️ I TRE BUFFER SONO GLOBALI, ED E' SICURO QUI E SOLO QUI. Il buffer dei
 ...
 *
 * ⚠️ IL TRIPLO INDIRETTO SI PERCORRE. Con blocchi da 1024 comincia oltre i
...
static void e2_percorri_blocchi(const unsigned char *ino_buf, unsigned int ino)
{
    ...
    /* --- triplo ---
     *
     * ⚠️ e2_ind1 SI RILEGGE A OGNI GIRO DEL CICLO ESTERNO, e non e' uno
                    ...
                    const char *percorso, int prof)
{
    ...
    /* ⚠️ SI CONTANO I RIFERIMENTI, non si marca "visto". Su ext2 un file
                             ...
                             const char *percorso, int prof)
{
    ...
    for (off = 0; off < dim; off += e2_dim_blocco) {
        ...
        unsigned char blocco[4096];         /* ⚠️ LOCALE: vedi sopra */
        ...
        while (p + 8u <= e2_dim_blocco) {
            ...
            /* ⚠️ UN rec_len DI ZERO NON AVANZA: il ciclo girerebbe per
 ...
 *
 * ⚠️ NON SI RIPARA UNA BITMAP "AGGIUSTANDO LE DIFFERENZE": si RISCRIVE da
 ...
 *
 * ⚠️ E PER QUESTO SI RIPARA SOLO SE NON C'E' NEMMENO UN PUNTO DI
 ...
 *
 * ⚠️ I BIT DI RIEMPIMENTO DELL'ULTIMO GRUPPO RESTANO A UNO. La bitmap di
...
static int e2_ripara_bitmap(void)
{
    ...
    for (g = 0; g < e2_n_gruppi; g++) {
        ...
        for (i = 0; i < e2_i_per_gruppo; i++) {
            ...
            /* ⚠️ Gli inode riservati sono occupati per definizione e non
...
static void e2_controlla_bitmap(void)
{
    ...
    /* ⚠️ QUANTI PROBLEMI ERANO GIA' NOTI PRIMA DI QUESTO GIRO. Una sola
    ...
    for (g = 0; g < e2_n_gruppi; g++) {
        ...
        /* ⚠️ La bitmap di un gruppo deve stare DENTRO quel gruppo: un
        ...
        if (e2_leggi_blocco(i_bm, bm) == 0) {
            for (i = 0; i < n_i; i++) {
                ...
                /* ⚠️ Gli inode riservati (1..primo_inode-1) sono occupati
    ...
    /* ⚠️ UNA SOLA RISCRITTURA SANA TUTTO QUELLO CHE E' STATO APPENA
    ...
    if (e2_incerto > 0) {
        printf("    ⚠️ NON riscrivo niente: ci sono %u punti in cui non ho\n",
               e2_incerto);
...
static void e2_controlla_link(void)
{
    ...
    for (i = 1; i <= e2_n_inode; i++) {
        ...
        /* ⚠️ Per una directory il conteggio comprende il suo "." e il ".."
...
/* ⚠️ Non si guarda la stringa "FAT16   ": e' decorativa. Si controllano i
...
int main(int argc, char **argv)
{
    ...
    /* ⚠️ SI SONDA, NON SI CHIEDE. Chi ha un volume malandato spesso non sa
