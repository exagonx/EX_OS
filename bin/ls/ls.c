 ...
 *
 * ⚠️ SU UN CD `.` E `..` NON CI SONO NEMMENO CON `-a`, e non e' un difetto
 ...
 * Impaginazione
 *
 * ⚠️ SI LEGGE UNA RIGA, NON UN TASTO. Leggere un singolo tasto vorrebbe
 ...
 *
 * ⚠️ LA PAUSA STA PRIMA DELLA RIGA, NON DOPO, e la differenza si vede
...
static void stampa_dettagli(const char *dir, const DirEntry *e, int con_attributi)
{
    ...
    /* ⚠️ SE stat() FALLISCE SI STAMPA LO STESSO. La voce esiste — ce l'ha
 ...
 *
 * ⚠️ SERVE DAVVERO PERCORRERLA DUE VOLTE, e solo per il modo a colonne: la
...
int main(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++) {
        ...
        /* ⚠️ SI RIFIUTA UN'OPZIONE SCONOSCIUTA invece di ignorarla. Un
    ...
    if (percorri(bersaglio, &r, 0, larghezza) != 0) {
        /* ⚠️ UN FILE NON E' UNA DIRECTORY, E NON E' UN ERRORE. listdir()
