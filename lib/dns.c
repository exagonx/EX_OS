 ...
 *
 * -----------------------------------------------------------------------------
 * ⚠️ LA COMPRESSIONE DEI NOMI E' IL PUNTO PERICOLOSO DI QUESTO FILE
 *
 ...
 *
 * -----------------------------------------------------------------------------
 * ⚠️ SI PRENOTA LA RICEZIONE PRIMA DI MANDARE
 *
                                ...
                                const char *nome)
{
    ...
    while (*s) {
        ...
        while (*s && *s != '.') {
            ...
            /* ⚠️ Un'etichetta oltre 63 byte non è rappresentabile: i due
 ...
 *
 * ⚠️ NON SEGUE I PUNTATORI, e non deve: un puntatore chiude sempre il nome,
        ...
        /* --- attesa: esito e dato possono arrivare in qualunque ordine --- */
        {
            ...
            while ((int)(uptime_ms() - scadenza) < 0) {
                ...
                for (k = 0; k < ancount; k++) {
                    ...
                    /* ⚠️ SI SALTANO I CNAME INVECE DI SEGUIRLI. Un server
