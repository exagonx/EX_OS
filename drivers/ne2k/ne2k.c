 ...
 *
 * -----------------------------------------------------------------------------
 * ⚠️ LA MASCHERA DELL'IRQ NON E' FACOLTATIVA
 *
 ...
 *
 * -----------------------------------------------------------------------------
 * ⚠️ IL BATTITO: PERCHE' SI GUARDA LA SCHEDA ANCHE SENZA INTERRUPT
 *
 ...
 *
 * -----------------------------------------------------------------------------
 * ⚠️ LE SCHEDE ISA NON SI SONDANO DA SOLE, E NON E' PIGRIZIA
 *
 ...
 *
 * ⚠️ IN PAROLE, QUINDI I BYTE DISPARI VANNO GESTITI. Il DCR e' impostato
...
static int inizializza_scheda(void)
{
    ...
    /* ⚠️ CURR = PSTART + 1 e BNRY = PSTART: uguali significherebbe
...
static int svuota_anello(void)
{
    ...
    for (giri = 0; giri < 64; giri++) {
        ...
        /* ⚠️ SI CONTROLLA `prossima` PRIMA DI USARLA. E' un byte scritto
 ...
 *
 * ⚠️ QUI SI REINIZIALIZZA TUTTA LA SCHEDA, E NON PERCHE' SIA ELEGANTE.
 *
...
static int trasmetti(const unsigned char *f, unsigned int len)
{
    ...
    /* ⚠️ RIEMPIMENTO A 60 BYTE. Sotto quella misura il frame non e'
...
static void servi_scheda(void)
{
    ...
    for (giri = 0; giri < 32; giri++) {
        ...
        /* ⚠️ SI AZZERA IL BIT PRIMA DI SVUOTARE, NON DOPO. Un pacchetto che
...
static void uso(void)
{
    ...
    printf("     ne2k.drv -i               stampa cosa ha trovato ed esce\n");
