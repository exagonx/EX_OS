 ...
 *
 * -----------------------------------------------------------------------------
 * ! MECCANISMO DI CONFIGURAZIONE #1 E ACCESSI A 32 BIT
 *
 ...
 *
 * ! 0xFFFFFFFF NON E' UN ERRORE. Quando in uno slot non c'è niente, il
 ...
 *
 * ! SI SCRIVE UNA WORD, NON UNA DWORD, E LA DIFFERENZA CONTA. A offset
 ...
 *
 * ! SOLO SU DISPOSITIVI CHE ABBIAMO ENUMERATO. Il controllo non è
...
static void stampa_elenco(void)
{
    ...
    if (g_scartati > 0) {
        printf("\n!  %d dispositivi trovati oltre il limite di %d e non elencati.\n",
...
{
    /* -i e' la convenzione comune a tutti i driver di EX-OS: sonda,
     * mostra cosa ha trovato ed esce con 0 se si applica a questa
     * macchina. E' cosi' che hwconfig sceglie quali driver installare
     * senza dover conoscere i singoli driver, e per il PCI la domanda ha
     * una risposta netta: qualcuno risponde sul bus, oppure no. */
    int sonda          = (argc > 1 && strcmp(argv[1], "-i") == 0);
    int elenca_ed_esci = (argc > 1 && strcmp(argv[1], "-l") == 0) || sonda;
    int rc;

    if (argc > 1 && !elenca_ed_esci) {
        printf("uso: pci.drv        avvia il servizio PCI\n");
        printf("     pci.drv -i     sonda il bus ed esce (0 = c'e' il PCI)\n");
    ...
    if (elenca_ed_esci) {
        stampa_elenco();
        /* Con -l si esce sempre bene: e' un elenco, e un elenco vuoto e'
         * un risultato valido. Con -i no — li' lo zero VUOL DIRE «questa
         * macchina ha un bus PCI», e su una macchina senza PCI nessuno
         * deve installare questo driver. */
        if (sonda) return (g_n_disp > 0) ? 0 : 1;
        return 0;
    }
