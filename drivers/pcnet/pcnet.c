 ...
 *
 * -----------------------------------------------------------------------------
 * ⚠️ QUESTA SCHEDA E' DIVERSA DALLA NE2000, E LA DIFFERENZA E' TUTTO
 *
 ...
 *
 * ⚠️ UN INDIRIZZO SBAGLIATO QUI NON DA' UN ERRORE. Dare alla scheda un
 ...
 *
 * ⚠️ IL RESET SI FA IN WIO. E' l'unica operazione che deve avvenire prima
 ...
 *      +12  riservato
 *
 * ⚠️ BCNT E' IN COMPLEMENTO A DUE, su dodici bit, e i quattro bit sopra
 ...
 *
 * ⚠️ OWN DICE DI CHI E' IL DESCRITTORE, e le due direzioni lo usano al
 ...
 * Anelli e buffer
 *
 * ⚠️ POTENZE DI DUE, E NON PER ELEGANZA: la scheda riceve il LOGARITMO
...
static int reset_e_dwio(void)
{
    ...
    /* ⚠️ In WIO: la scheda appena accesa e' li', e l'offset del RESET e'
    ...
    /* ⚠️ SI VERIFICA CHE IL CAMBIO ABBIA PRESO. Dopo un reset CSR0 vale
...
static int inizializza_scheda(void)
{
    ...
    /* ⚠️ SI ASPETTA IDON, E CON UNA SCADENZA. La scheda deve andare a
...
static void accoda(const unsigned char *f, unsigned int len)
{
    ...
    if (g_coda_conta == CODA_N) {
        /* ⚠️ SI BUTTA IL PIU' VECCHIO. Una coda piena significa che
...
static int trasmetti(const unsigned char *f, unsigned int len)
{
    ...
    /* ⚠️ SI RIEMPIE FINO A 60 BYTE. Sotto quella soglia il frame non e'
    ...
    for (n = 0; n < 16; n++) {
        ...
        for (i = 0; i < 6; i++) {
            if (d.bar_io[i] && d.bar[i] != 0) {
                ...
                /* ⚠️ QUI IL BUS MASTER NON E' FACOLTATIVO, e questa e' la
...
{
    printf("uso: /dev/pcnet.drv [-l] [-p PORTA -q IRQ]\n\n");
    printf("  -l        mostra la scheda trovata ed esce\n");
...
/* =============================================================================
 * ⚠️ SONDARE UNA SCHEDA GIA' GUIDATA LA SPEGNE
 *
...
{
    DmaZona z;
    int     solo_elenco = 0, i, rc;

    memset(&g_cont, 0, sizeof(g_cont));

    for (i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-l") == 0)      solo_elenco = 1;
    ...
    /* ⚠️ LA MEMORIA SI CHIEDE PRIMA DI TOCCARE LA SCHEDA. L'init block
