...
#
# -----------------------------------------------------------------------------
# ⚠️ PERCHE' ESISTE: UN CAMBIO DI ABI NELLA LIBC NON SI VEDE, SI SUBISCE
#
...
# ⚠️ Il PATH PRIMA di tutto, --verifica compreso: l'impronta la calcola
...
#
# ⚠️ Le dimensioni le calcola il COMPILATORE DEL BERSAGLIO e si leggono
...
# ⚠️ E' una DICHIARAZIONE, e vale quanto chi la fa: dirla senza aver
...
#
# ⚠️ SI CANCELLANO LE DIRECTORY, non si fa `make clean`: gli header del
...
if vuoi libgcc; then
    ...
    # ⚠️ all-gcc PRIMA e a -j1, ESPLICITO. Le librerie del bersaglio lo
...
#
# ⚠️ Il configure va RIFATTO, non solo il make: libiberty compila una
...
#
# ⚠️ DIRECTORY NUOVA. Non e' prudenza: --enable-checking=release cambia
...
if vuoi openssl; then
    ...
    # ⚠️ libcrypto.a E BASTA, non `build_libs`. Quello tira dentro anche
