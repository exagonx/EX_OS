 ...
 *
 * -----------------------------------------------------------------------------
 * ⚠️ LA RACE DEL RISVEGLIO PERDUTO, e come la si evita QUI
 *
 ...
 *
 * ⚠️ Qui e' peggio che con la tastiera, perche' chi sveglia non e' un
 ...
 *
 * -----------------------------------------------------------------------------
 * ⚠️ IL BUFFER DI RIMBALZO, e perche' non se ne puo' fare a meno
 *
...
/* ⚠️ CHIUDERE UN'ESTREMITA' E' ANCHE UN RISVEGLIO. Chi dorme dall'altra
...
int32_t pipe_leggi(int h, void *buf, uint32_t n)
{
    ...
    for (;;) {
        ...
        /* ⚠️ VUOTA E SENZA SCRITTORI = FINE DEI DATI. Vuota CON scrittori =
     ...
     * ⚠️ La copia dall'anello avviene ADESSO, non dopo: uno scrittore
...
int32_t pipe_scrivi(int h, const void *buf, uint32_t n)
{
    ...
    for (;;) {
        ...
        /* ⚠️ NESSUN LETTORE = EPIPE, non attesa: quei byte non li leggera'
     ...
     * ⚠️ LA SCRITTURA PUO' ESSERE PARZIALE, ed e' corretto: write() ritorna
     ...
     *
     * ⚠️ I byte entrano nell'anello PRIMA che `quanti` salga: alzare il
