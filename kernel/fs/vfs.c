 ...
 *
 * ! CHI LO PRENDE NON DEVE POTER FAULTARE. Se un driver scrivesse dentro
 ...
 *
 * ! E' LA DIFFERENZA FRA FUNZIONARE E NO, non un'ottimizzazione. La
...
void vfs_init(uint8_t boot_drive)
{
     ...
     *
     * ! IL DRIVE DI AVVIO NON LO DICE. Con El Torito in emulazione floppy
     ...
     *
     * ! NON E' UN'EURISTICA: "il controller non risponde" e' esattamente
    ...
    if (!fat12_pronto()) {
        ...
        for (i2 = 0; i2 < n; i2++) {
            ...
            g_mnt[0].mnt          = mnt;
            g_mnt[0].blkdev       = i2;
            /* ! SOLA LETTURA, e non e' una scelta: un ISO 9660 non si
...
static int apri_nl(const char *abs, uint32_t flags, int autorita)
{
    ...
    /* ! LO SLOT SI PRENOTA QUI, non a fine funzione.
     *
 ...
 * che non darlo.
 *
 * ! NON ERA PRUDENTE, ERA UNA RISPOSTA SBAGLIATA DETTA CON SICUREZZA.
 ...
 * il primo cluster.
 *
 * ! IL +1 SERVE ALLE RADICI. Un punto di montaggio non ha una voce di
 ...
 *
 * ! IL LIMITE, detto: su FAT due file vuoti hanno lo stesso primo
                ...
                uint32_t *count, uint32_t start)
{
    ...
    *count = 0;
    ...
    for (i = 0; i < VFS_MAX_MOUNT && scritti < max; i++) {
        ...
        /* La RADICE del volume montato li' sopra, e la sua identita' e'
         * quella che stat_interno() da' a un punto di montaggio: lo zero
         * del proprio volume. Vedi il ! sul +1 accanto a VFS_IDENT. */
        out[scritti].ident      = VFS_IDENT(i, 0);
        ...
        if (g_mnt[im].tipo == VFS_FS_FAT12FD) {
            while (scritti < max) {
                ...
                for (j = 0; j < n; j++) {
                    ...
                    out[scritti].ident      =
                        VFS_IDENT(im, (uint32_t)tmp[j].first_cluster + 1u);
                    out[scritti].is_dir =
            ...
            while (scritti < max) {
                ...
                for (j = 0; j < (uint32_t)n; j++) {
                    ...
                    out[scritti].ident      = VFS_IDENT(im, tmp[j].extent + 1u);
            ...
            while (scritti < max) {
                ...
                for (j = 0; j < (uint32_t)n; j++) {
                    ...
                    v_copia(out[scritti].nome, tmp[j].nome,
                            sizeof(out[scritti].nome));
                    out[scritti].dimensione = tmp[j].dimensione;
                    out[scritti].ident      = VFS_IDENT(im, tmp[j].inode + 1u);
                    out[scritti].is_dir     = tmp[j].is_dir;
                    scritti++;
                }
                s_fs += (uint32_t)n;
            ...
            while (scritti < max) {
                ...
                for (j = 0; j < (uint32_t)n; j++) {
                    ...
                    out[scritti].dimensione = tmp[j].dimensione;
                    out[scritti].ident      =
                        VFS_IDENT(im, tmp[j].primo_cluster + 1u);
                    out[scritti].is_dir     = tmp[j].is_dir;
                    scritti++;
                }
                s_fs += (uint32_t)n;
 ...
 *
 * ! SOLO NELLA STESSA DIRECTORY E NELLO STESSO MONTAGGIO. Tutto il resto
 ...
 *
 * ! E LA GARANZIA E' PROPRIO IL PUNTO: I DATI NON SI SPOSTANO. Si
...
static int vfs_rename_nl(const char *da, const char *a)
{
    ...
    /* ! fat12.c RISPONDE GIA' IN errno, e non va tradotto. Gli altri due
