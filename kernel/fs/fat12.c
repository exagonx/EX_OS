...
static Fat12DirEntry *fat12_find_path(const char *path, Fat12DirEntry *out)
{
    ...
    /* Cerca nella root dir */
    path_to_fat83(path, name83);
    entry = fat12_find_in_root(name83);
    if (entry) {
        if (out) { uint8_t k; uint8_t *d=(uint8_t*)out, *s=(uint8_t*)entry;
                   for(k=0;k<sizeof(Fat12DirEntry);k++) d[k]=s[k]; }
        return entry;
    }

    /* Prova con subdirectory: cerca il primo componente nella root */
    const char *p = path + 1;  /* salta '/' iniziale */
    const char *slash = p;
    while (*slash && *slash != '/') slash++;
    if (*slash != '/') return NULL;  /* nessuna subdir */
