...
static void print_system_info(BootInfo *info)
{
     ...
     * "Stage 2 ci ha provato e non ce l'ha fatta" si distinguono solo qui,
     * e chi ha appena chiesto 1024x768 vuole sapere quale dei due e'. */
    if (info->fb_addr != 0)
        klog(LOG_INFO, "  Video         : %ux%u %u bpp, framebuffer 0x%08x, "
                       "%u byte/riga",
             info->fb_width, info->fb_height, info->fb_bpp,
             info->fb_addr, info->fb_pitch);
    else
        klog(LOG_INFO, "  Video         : modo testo 80x25");
...
klog(LOG_INFO, "[PASSO 9] Paginazione OK (CR0.PG=1)");
    ...
    /* =====================================================================
     * PASSO 9a: console grafica, se Stage 2 ha impostato una modalita' VESA
     *
     * ! QUI E NON PRIMA. Il framebuffer sta fuori dalla RAM e va mappato,
     * quindi serve la paginazione appena accesa al passo precedente. E qui
     * e non dopo: da questo punto in avanti ogni klog dei passi successivi
     * si vede a schermo invece di finire in una memoria video che non si
     * sta guardando.
     *
     * Senza modalita' grafica non fa niente e la console resta 80x25.
     * ===================================================================== */
    vga_init_grafica(info);
...
/* =========================================================================
    ...
    if (info->boot_drive < 0x80) {
        /* ! SI SONDA, NON SI INIZIALIZZA A FORZA. Il fallimento qui NON e'
     ...
     *
     * ! ORDINE CAMBIATO nella 0.134, e non e' un riordino cosmetico.
     *
...
KernelConfig *cfg = cfg_load();
     ...
     * cio' che non deve succedere in silenzio.
     * ===================================================================== */
    if (cfg->svga[0] != '\0') {
        const char *reale = "testo";

        if (info->fb_addr != 0) {
            if      (info->fb_width == 640)  reale = "640x480";
            else if (info->fb_width == 800)  reale = "800x600";
            else if (info->fb_width == 1024) reale = "1024x768";
            else                             reale = "(altra)";
        }
        if (!str_equal(cfg->svga, reale)) {
            klog(LOG_WARN, "[PASSO 13b] kernel.cfg chiede svga=%s, l'avvio e' "
                           "partito in %s", cfg->svga, reale);
            klog(LOG_WARN, "            Il kernel non puo' cambiarla: la "
                           "imposta Stage 2. Allinea con  svga %s", cfg->svga);
        }
    }
    ...
    if (cfg->mount_count > 0) {
        ...
        for (i = 0; i < cfg->mount_count; i++) {
            ...
            /* ! GIA' MONTATO NON E' UN GUASTO, ED ERA IL CASO PIU' COMUNE
    ...
    int kbd_drv_loaded = 0;
    {
        ...
        for (di = 0; di < CFG_MAX_MODULES; di++) {
            ...
            int rc_drv = elf_load_residente(cfg->modules[di].path, drv_proc,
                                            &drv_res);
            if (rc_drv == 0) {
                ...
                /* Il codice va detto qui per la stessa ragione del PASSO 15:
         ...
         (unsigned)VGA_N_CONSOLE, cfg->shell_path);
    {
        ...
        for (n = 0; n < VGA_N_CONSOLE; n++) {
            ...
            /* ! SE C'E' UN LOGIN SI LANCIA QUELLO, non la shell. Non e'
             * un livello in piu' per gusto: login e' un CICLO che rilancia
             * la shell ogni volta che finisce, ed e' cosi' che `exit`
             * smette di lasciare la console morta. Il kernel lancia una
            ...
            if (rc_shell == 0) {
                 ...
                 *
                 * ! IL CODICE VA RIPETUTO QUI. Dal momento in cui «file
                 * assente» e' sceso a LOG_INFO — perche' cercare nel PATH
                 * lo produce a ogni comando sbagliato, vedi elf_carica —
                 * la riga ELF "qui sopra" non c'e' piu' proprio nel caso
                 * in cui questo avviso e' fatale. err=-2 e' ENOENT. */
