...
- `sched_switch_to` fa `fpu_save(prev)` e `fpu_restore(next)` **sempre**,
  ...
  pigro si sbaglia.
- ⚠️ Zero **non** e' uno stato valido per `FRSTOR`: parola di controllo a
...
⚠️ **Trappola per chi scrive programmi, non per il kernel.** GCC valuta le
...
⚠️ `Stat.st_first_clus` resta 0 fuori da una FAT, ed e' voluto: e' un
...
⚠️ **Prezzo della mappa: ricopiare kernel o Stage 2 sul disco OBBLIGA a
...
⚠️ **LA PARTIZIONE ATTIVA VA SCELTA CON LO STESSO CRITERIO IN DUE POSTI.**
...
### ⚠️ Ordine di inizializzazione cambiato (0.134)
...
⚠️ **LA CACHE E' WRITE-BACK: LO SFRATTO DI UNO SLOT SPORCO DEVE RIVERSARLO.**
...
⚠️ **LE COPIE DELLA FAT VANNO AGGIORNATE TUTTE.** Scriverne una sola non
...
⚠️ **IL CLUSTER SI MARCA OCCUPATO PRIMA DI RESTITUIRLO.** Fra "trovato
...
⚠️ **IL CONFINE DEL PREFISSO: `/disk2` NON STA DENTRO `/disk`.** E' l'errore
...
### ⚠️ kernel.cfg troncato in silenzio — kernel/fs/cfg.c (2026-07-31, 0.132)
...
⚠️ **Per toccare il caso della voce FAT12 a cavallo di due settori serve un
...
⚠️ **L'overflow si controlla PRIMA della somma**: `lba + n` puo'
...
### ⚠️ VirtualBox: il disco va sul controller IDE, non SATA
...
⚠️ QEMU non emula HPA: il ramo "clippato" **non e' mai stato eseguito**.
...
### ⚠️ Il FAT12 attuale NON e' una base per FAT16/FAT32
...
**⚠️ Prossimo candidato al risparmio**: `KERNEL_STACK_SIZE` è **128 KB per
...
**⚠️ IL PUNTO DA NON PERDERE — perché si servono anche i fault da ring0.**
...
⚠️ `uptime_ms()` torna a zero dopo ~24,8 giorni: confrontare sempre
...
### ⚠️ REGOLA: non si può aspettare meno di un tick (2026-07-31)
...
### ⚠️ APERTO: tetto di 64 MB sulla RAM — manca E820 in Stage2
