...
⚠️ **The extra drivers do not go there.** Networking (`pci`, `ne2k`,
...
> ⚠️ **USB floppy**: the bootloader goes through the BIOS (INT 13h) and works,
...
> ⚠️ There is no **dynamic** TLS (`__tls_get_addr`, thread-local variables
...
> ⚠️ A large enough heap would have remapped **the process's own TLS block**
...
> ⚠️ The result: two ESC keys pressed by mistake, two Backspaces, and the two
...
⚠️ In QEMU the RTC starts in **UTC**, not local time. To see the time in your
...
⚠️ The price is the same bargain LILO made: **copying a new kernel or Stage 2
...
> ⚠️ **A missing primitive was needed.** The swap could not be done:
...
> ⚠️ The rule is **"the source is newer"**, not "the dates differ". Copying a
...
> ⚠️ **It only works on an unmounted partition**, and that is not an
...
> ⚠️ **Without `-r` it does not write a single sector.** A checker that
...
> ⚠️ Whoever renames by touching only the 8.3 entry — and **EX-OS's `rename`
...
⚠️ ext2 has no journal and this driver does not invent one: an interruption
...
⚠️ **`x == 0.025` can be false even when `x` is right.** On x87 GCC evaluates
...
> ⚠️ **Whoever uses these functions must link `-lm`.** The exceptions are
...
> ⚠️ **The returned pointer is released with `free()`**, not with a special
> free: to the heap it is a block like any other, and merging with the
> neighbours works without knowing any of this.

> ⚠️ `posix_memalign` **returns** the error code and does not put it in
...
> ⚠️ **Empty with a live writer = wait; empty with no writers = 0.** That is
...
> ⚠️ **No `SIGPIPE`**: EX-OS has no signals, so only the return value is
...
close(p[1]);                            /* ⚠️ indispensable */
```

> ⚠️ **The parent must close the end it handed over.** If it does not, the
...
> ⚠️ **It is called `rename` and not `mv`, on purpose.** `mv` on Unix renames
...
> ⚠️ **Two differences from POSIX, declared:** same **directory** only
...
> ⚠️ **`fat12.c` answers in errno, the other two do not.** There `-2` means
...
> ⚠️ **`getrusage` is not a measurement.** EX-OS keeps no per-process
...
> ⚠️ **`mmap` maps anonymous memory only**: with `fd != -1` it gives
...
> ⚠️ **The two descriptors share the file, not the position.** On POSIX a
...
> ⚠️ **Exceptions work, and that was not a given.** They are the piece that
...
> ⚠️ **A missing name is a compile error, not a dead branch.**
...
⚠️ **Without them the driver only gets halfway.** `gcc -c` needs only cpp,
...
⚠️ **The 64-bit division is there on purpose.** It is one of the few things
...
**⚠️ The binary has to be rebuilt.** The test was done before the move to a
...
⚠️ **The confirmation is in the symbol table, not in the reasoning.** Inside
...
> ⚠️ **It is the same defect as `cc1`'s, not another one.** There I caught it
...
> ⚠️ **The relinked `ld` worked anyway**, and that is the instructive part:
...
⚠️ The CD is read-only by construction, so headers and libraries are read
...
⚠️ **The previous files go to `.bak`**, and that is what makes the proposal
acceptable: if the machine does not boot, the earlier one is right there.

⚠️ **The new file is generated, not edited.** The `kernel.cfg` that ships
...
⚠️ **The state is why it exists.** A list of commands to type is already in
...
⚠️ `ioport_in32` **does not return the value read**: `0xFFFFFFFF` («no
...
⚠️ `irq_done()` is not optional. The kernel **masks** the IRQ in the PIC
...
⚠️ An **ISA** NE2000 is not probed for on its own: recognising it would mean
...
⚠️ **The difference from the NE2000 is everything.** That one keeps the
...
> ⚠️ **A wrong address here does not give an error.** Giving the card a
...
⚠️ **`notifiche IRQ 7` against 7 frames is the line that counts**, not
...
⚠️ **`-l` does not probe a card someone else is driving.** Reading its state
...
⚠️ `ping` distinguishes **three** outcomes, not two: reply received; no
...
⚠️ A time of `<10 ms` is not a zero: `uptime_ms()` counts PIT ticks at
...
⚠️ **The checksum is not optional.** Every long-name entry carries the
...
⚠️ **Reading only.** Creating a file with a long name would mean allocating
...
⚠️ **ASCII only**: characters above `0x7F` become `?`. EX-OS has no character
...
⚠️ **ext2 never enters the automatic choice**: it is a format you ask for,
...
⚠️ **It does not renew the lease.** When it expires, run it again. Renewal
...
⚠️ A datagram for an open port with nobody waiting is **dropped and counted**
...
⚠️ **Significant digits stop at 18, and zeros are printed beyond.** It is a
...
⚠️ **Rounding is to even**, as the standard prescribes: `%.0f` of 2.5 gives
...
⚠️ And `gettimeofday` now takes **seconds and microseconds from the same
...
⚠️ **Only the FIRST console's shell runs it.** EX-OS starts one for each of
...
⚠️ **The way out exists before it is needed.** An autoexec containing a
...
⚠️ **It silences the command, not its output**, and that is the distinction
...
⚠️ **`source` and not a spawn**: the commands must run in the current shell,
...
⚠️ **The line in progress is not lost.** Whoever has typed half a command and
...
⚠️ **The keyboard's raw mode is needed**, because in cooked mode the driver
...
⚠️ **If the `kbd` service does not answer, it goes back to reading whole
...
⚠️ **Only the foreground console** gets the keys. Without that check all four
...
⚠️ The redraw uses **Backspace only**, because EX-OS's TTY has no cursor
...
⚠️ **Single and double quotes do the same thing.** On a Unix shell the
...
⚠️ **Here `-d` means «details»**, not what it means on Unix (where `ls -d`
...
⚠️ Without `-a`, names beginning with a dot are hidden, `.` and `..`
...
⚠️ FAT's «hidden» bit is **shown** with `-md` but hides nothing. Looking at
...
⚠️ A zero date **means «this volume does not keep them»** and programs print
...
⚠️ **Outgoing connections only.** The `LISTEN`/`SYN_RECEIVED` branch of the
...
⚠️ **Sequence numbers are compared by subtraction, never with `<`.** They are
...
⚠️ `IP_MSG_TCP_APRI` can answer **`-EAGAIN`**: it means the stack has just
...
⚠️ **Passive mode (`PASV`) only.** In active mode it is the *server* that
...
⚠️ **FTP sends the password in the clear.** It is not a defect of the
...
⚠️ If the server announces in `PASV` an address different from the one we are
...
⚠️ **Name-compression pointers are followed only when reading, with a cap on
...
⚠️ `ping` prints the name **and** the address when given a name: without it,
...
| **exFAT** | ⚠️ it does not exist as a filesystem: it has to be implemented first, and only then does a chkdsk make sense |
