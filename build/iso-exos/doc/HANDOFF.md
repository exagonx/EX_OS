...
! **Non si era mai visto** perché i programmi di EX-OS sono in C e la
...
! **18 è un numero MISURATO**: fino a 18 nessuna discordanza, a 19 la
...
! `as` e `ld` nativi **non** sono toccati: sono già linkati con la libc
...
- **I file creati da EX-OS non avevano data**: `fat12.c` scriveva
  ...
  ! Se il CMOS non risponde si lascia zero: una data inventata sarebbe
...
! **Il dispatch dei comandi è stato ESTRATTO dal ciclo principale della
...
! **Solo la shell della console 0.** EX-OS avvia una shell per ognuna
...
! **La condizione è «la console», non «sono la prima shell»**, ed è
...
! **La via d'uscita esiste prima di servire**: `autoexec=0` in
...
byte-identico sull'host. ! **La sola dimensione non avrebbe provato
...
! **Solo modo passivo**, per la ragione detta in `ip_proto.h`: il modo
...
! **Le risposte FTP possono essere su più righe.** La regola è il
...
! **Del `PASV` si cercano i numeri, non si segue un formato.** Il testo
...
! **Se il `PASV` annuncia un indirizzo diverso da quello a cui siamo
...
! **La password in chiaro si dice all'accesso**, una volta. È il
...
floppy, e niente `/dev/pci.drv`. ! Le istruzioni della catena di rete
...
! **L'intestazione TCP può essere più lunga di 20 byte.** Le opzioni
...
! **Il FIN va dopo i dati in coda.** Il suo numero di sequenza è quello
...
! **Non si conferma quello che non si può tenere.** Se il buffer di
...
! **`IP_MSG_TCP_APRI` può rispondere `-EAGAIN`**, e non è un fallimento:
...
! **`IP_MSG_TCP_INVIA` può accettare MENO byte di quelli offerti**, e il
...
! **Si risponde a `IP_MSG_TCP_RICEVI` anche con zero byte**, quando la
...
! **Solo chi ha aperto la connessione può usarla.** L'identificativo è un
...
! **MMX non aveva bisogno di niente**, ed è il punto meno ovvio. I
...
! **Servono `FXSR` E `SSE`, che in CPUID sono bit distinti** — proprio
...
! **Senza OSXMMEXCPT un errore SIMD arriva come #UD**, cioè come
«istruzione inesistente» su un'istruzione che esiste benissimo: la
diagnosi peggiore possibile.

! **MXCSR va scritta a mano nel modello pulito.** `FNINIT` azzera l'x87
...
! **FXSAVE pretende l'allineamento a 16.** Il campo `fpu_state` del PCB
...
! **L'ordine delle domande è obbligato**: non si può chiedere a CPUID se
...
! **E QUI VA CORRETTA UNA PREMESSA**: il kernel **non gira nei 640 KB di
...
! **Non è memoria**: identico con 256 MB e con 512 MB. È qualcosa che
...
! **La somma di controllo non è facoltativa.** Ogni voce LFN porta la
...
! **Si confronta col nome lungo se c'è, altrimenti con l'8.3 — e se il
...
! **L'azzeramento dell'accumulatore va fatto anche sulle voci SALTATE
...
## ! MI ERO SBAGLIATO SULLE VIRGOLETTE
...
! **Un carattere non mappato fa cadere lo script a metà prova, e la prova
...
! **Non dava nessun errore: dava un risultato sbagliato che sembrava
...
! **E la variabile è definita in TESTA al Makefile**, non accanto alla
...
- **Un'opzione sconosciuta viene rifiutata**, non ignorata. Un `ls -l`
  ...
  l'opzione esista.
- ! **La pausa dell'impaginazione sta PRIMA della riga successiva, non
...
! **Si indica UN passo solo.** Elencare tutti i comandi mancanti sembra
...
## ! La compressione dei nomi è il punto pericoloso del file
...
! Aggiungere `dns.o` a `ipcfg` e `nettest` non costa: si compila con
...
! **Non ho rilinkato il `cc1` esistente con la libc corretta.** Il
...
! Il controllo **serve** agli altri chiamanti: `free()` e `memalign()`
...
! Il build con `--enable-checking=release` è **in corso** in
...
! **La somma di controllo UDP copre una pseudo-intestazione** di dodici
...
! **Somma a zero in arrivo si accetta**: su IPv4 il controllo è
...
! **Parte azzerando l'indirizzo**, perché un client DHCP deve mandare con
...
! **Alza il bit di broadcast** nel campo `flags`: l'indirizzo che il
server sta per darci non ce l'abbiamo, e non risponderemmo a un ARP per
quell'indirizzo.

! **Controlla `xid` e `chaddr`** su ogni risposta. Le risposte agli altri
...
! **La prima prova DHCP non provava niente**, e me ne sono accorto solo
...
! **La conferma era già nei log e non l'avevo letta.** Nel dump `info
...
### ! E la mia "prova" della sessione scorsa era sbagliata
...
! **`ping` non stampa mai «0 ms».** `uptime_ms()` conta i tick del PIT a
...
! **`ttl=64` è la prova.** È il TTL che imposta il nostro stack; lo slirp
usa 255. Quelle risposte le ha composte `rispondi_echo` sulla macchina A,
e l'ARP l'ha risolto `arp_rispondi`.

! **`EXOS_MAC` non è un dettaglio.** Il primo tentativo aveva le due
...
! **40 MB non entrano in 32 MB di RAM.** Il build ha
...
! Vale per **tutti** i driver ring3, tastiera compresa: `kbd.drv` ora
...
! **Le schede ISA non si sondano.** Per riconoscerne una bisogna scrivere
...
! `PCI_MSG_DISABILITA` non è la coppia decorativa di `ABILITA`: è il
...
! **Non carica niente, per ora.** «Quale driver assegnare» ha due metà,
...
! **Le NE2000 ISA non si vedono da qui** e non è un difetto: non hanno
...
! Su ext2 non si vedeva: li' la mappa regge 12 intervalli. Quattro prove
...
! Solo stessa directory (ENOSYS) e non sostituisce (EEXIST). In
...
! **`fat12.c` risponde in errno, gli altri due no**: li' `-2` e' ENOENT,
...
! Solo su partizione **smontata** (sopra una montata c'e' una cache
...
! La riparazione delle bitmap ext2 **non parte se c'e' un solo punto di
...
## ! La marcatura GPLv3 impediva a un file di compilare
...
! **Il checksum del catalogo e' su parole da 16 bit e la somma deve fare
...
## ! MA IL KERNEL NON PUO' LEGGERE IL FLOPPY EMULATO
...
| il kernel | NO: modo protetto, parla al controller: trova porte vuote |
...
! Richiede di togliere l'inchiodatura del montaggio 0, che oggi e' voluta
...
! **Il segnale non e' il numero del drive**, che con l'emulazione vale
...
! Nel Makefile sta scritta la conseguenza che decide il contenuto: **il
...
! **Il CD serve a installare**: la radice in sola lettura non e' un limite
...
! Due prove di `libctest` fallivano da CD: e' stata corretta **la prova**,
...
! **Il kernel deve poter FERMARE la scheda**, non solo assegnarle il DMA:
...
! Userspace da' **riavviabilita', non contenimento**: senza IOMMU un
processo che programma un bus master puo' far scrivere ovunque.

! **NE2000 non e' un bus master** — si programma a porte — quindi il suo
...
| Funzione | ! Il limite, dichiarato |
|---|---|
| `getpagesize` | nessuno: 4096 |
| `getrusage` | ! **non e' una misura**: `ru_utime` e' il tempo TRASCORSO dall'avvio, non il tempo di CPU del processo — EX-OS non tiene contabilita' per processo. Tutto il resto e' zero |
| `mmap` | ! **solo memoria anonima**: `fd != -1` da' `ENODEV` |

! **`getrusage` riporta un limite superiore, non zero**, ed e' una scelta:
...
! **`mmap` di un file si rifiuta invece di consegnare zeri.** Servirebbero
...
! E ritorna **`MAP_FAILED`, cioe' `(void *)-1`, non `NULL`**: e' la
...
### ! E una prova mia mal specificata
...
! **`FD_PIPE_R` e `FD_PIPE_W` sono due TIPI, non uno con un flag.** La
...
! **Vuota con scrittori vivi = aspetta; vuota senza scrittori = 0.** E'
...
! **Niente garanzia di atomicita' di POSIX**: la scrittura puo' essere
...
! **La magia di `SpawnExtra` e' passata da `'SPNX'` a `'SPNY'`** perche' e'
...
## ! LO STALLO CHE HA RIVELATO UN DIFETTO DI PROGETTO
...
! **E' il motivo per cui su Unix i descrittori si chiudono in `do_exit()`
...
! **Servono le varianti `_locked`**: `proc_exit`/`proc_kill` girano gia'
...
! **Non e' un'incognita: e' un difetto della prova.** i386 e'
...
## ! E UN DIFETTO VERO: `struct stat` AVEVA I TIPI SBAGLIATI
...
! **Sul nostro bersaglio hanno la stessa larghezza, quindi i VALORI erano
...
! **`st_blksize` vale 512 e non 4096**: e' il SETTORE, che e' l'unita'
...
! **`st_size` ora e' SEGNATO** (`off_t` e' `long`), quindi il tetto e'
...
## ! Le eccezioni funzionano, e non era scontato
...
! Le eccezioni si abilitano **da sole**: `-fno-exceptions -fno-rtti` non
...
! **Nessuno di questi si e' scoperto leggendo la documentazione**: si sono
...
3. ! **lo spazio**: `cc1` per x86-64 spogliato e' 50 MB, per i386 sara'
...
rm -rf ~/gcc-build-cxx/i386-exos/libstdc++-v3     # ! la cache di configure
cd ~/gcc-build-cxx
...
! **Si compila con `g++` e si collega con `gcc`**, ed e' la descrizione
...
! **Niente distruttore virtuale**, ed e' il confine esatto: un
...
## ! LA RIGA CHE MANCAVA DA SEMPRE — `extern "C"`
...
! **E il sintomo era fuorviante.** Non e' arrivato al collegamento: e'
...
## ! `sleep()` ritornava `void`, e POSIX dice `unsigned int`
...
! **Un nome mancante e' un errore di compilazione, non un ramo morto.**
...
! `EWOULDBLOCK` **e'** `EAGAIN` e `ENOTSUP` **e'** `EOPNOTSUPP`, stesso
...
! **normalizza la struttura che riceve**, che e' meta' del suo lavoro:
`tm_mon = 12` diventa gennaio dell'anno dopo, `tm_sec = 90` diventa un
minuto e mezzo. Per questo il parametro non e' `const`.

**`localeconv` e `struct lconv`.** ! I campi non specificati valgono
...
! **Dopo il primo errore non si e' andati a tentativi.** L'elenco di cio'
...
! **Il diff con `grep -w` da' falsi negativi**: `mktime` risultava
...
## ! LA TRAPPOLA PIU' COSTOSA — sei macro mancanti, 136 errori lontanissimi
...
! **Tutte quelle funzioni ESISTONO nella nostra libc.** Il messaggio non
...
! **Non sono sinonimi di `<`, `<=`, `>`, `>=`.** Con un operando NaN danno
...
## ! E LA CACHE DI configure ANDAVA BUTTATA
...
! **Si cancella la directory di build di libstdc++, NON si rilancia il
...
## ! `nearbyintl` MANCA A openlibm 0.8.7 — e costa cara
...
! **La differenza fra `rintl` e `nearbyintl` e' UNA SOLA**: danno lo stesso
...
### ! Il difetto che ci ho messo dentro io, e come si e' visto
...
### ! E un errore di conduzione: due membri omonimi in un archivio
...
## ! I DUE DIFETTI CHE SI TENEVANO NASCOSTI A VICENDA
...
! **`heap_ultimo` non e' «l'ultimo blocco prima del break»**: e' l'ultimo
...
! **L'intestazione resta dov'e'** e il blocco continua a esistere, solo
...
! **NON SI E' ANDATI A TENTATIVI.** Dopo il primo errore l'elenco e' stato
...
| Aggiunto | Nota |
|---|---|
| `div_t`, `ldiv_t`, `lldiv_t`, `div`, `ldiv`, `lldiv` | ! troncamento verso **zero**: `div(-7,2)` = (-3, -1) |
| `llabs`, `atoll` | |
| `rand`, `srand`, `RAND_MAX` | ! LCG del K&R — **non** e' casuale, non per chiavi |
| `mblen`, `mbtowc`, `wctomb` | ! ritornano `int`, non `size_t` come le `mbr*` |
| `strxfrm` | ! ritorna la lunghezza dell'**originale**, non del copiato |
| `fpos_t`, `fgetpos`, `fsetpos` | ! ritornano 0/-1, non la posizione |
| `setbuf`, `setvbuf` | ! **setvbuf dice di no** invece di fingere |
| `vscanf`, `_Exit`, `at_quick_exit`, `quick_exit` | |
| `difftime`, `struct timespec`, `timespec_get` | ! `timespec_get` ritorna **`base`**, non 0 |
| `system` | ! **ritorna sempre -1 con ENOSYS** |

### ! `lldiv` non poteva usare l'operatore `/`
...
! Il resto prende il segno del **dividendo**, non del divisore, e il
...
### ! Le due funzioni che dicono di no invece di fingere
...
! **`system()` c'e' ma non esegue niente, e lo dice.** Non e' un
...
## ! Una trappola nella conduzione, non nel codice
...
presente senza dire niente**. ! Uno heap che avesse superato `tls_base`
...
## ! Due cose che avevo detto e che il codice ha smentito
...
! **Il controllo sta prima del ciclo, non dentro.** Fermarsi a meta'
...
! **`MAP_FIXED` non puo' piu' prendersi la zona alta.** Su POSIX
...
! **Il confronto e' scritto per non traboccare**: `heap_max - heap_end`
...
## ! La prova non raggiunge il tetto, e non puo'
...
! La prova **restituisce** la memoria con `sbrk` negativo, e non e'
...
! **I valori sono moltiplicati per mille e stampati come interi**: la
...
## ! Perche' una libm DI TERZI, dopo aver scritto per un anno che non ce n'era una
...
`lib/include/math.h` e' stato riscritto (283 righe). ! **Le dichiarazioni
...
! **`sqrt` resta nella libc e la doppia definizione NON e' un problema.**
...
## ! `ARCH=i387` e `OS=Linux`, e nessuno dei due e' una bugia
...
## ! LA TRAPPOLA DELLA SESSIONE — `type` e' una parola che non ci appartiene
...
! **La colpa non e' di openlibm.** Un header pubblico non puo' usare come
...
! **La conseguenza che conta: il puntatore restituito si libera con
...
## ! La seconda trappola — `.gitignore` si era mangiato roba nostra
...
! **`--with-newlib` per la libstdc++ non vuol dire "usa newlib"**: vuol
...
! E **`--enable-clocale=generic` sulla riga di configure**, perche'
...
! **Riconfigurare in una directory di build gia' fatta ricompila TUTTO
...
! Nell'ordine `-lmpc -lmpfr -lgmp` e non l'inverso: `ld` scorre gli
...
| `sqrt` | `src/eta.c` di MPC | ! vedi sotto |
| `_STDIO_H` | `gmp.h` | ! una macro, non una funzione |

! **`sqrt` entra in `<math.h>` senza contraddire quello che c'e' scritto
...
## ! Un binario di EX-OS parte anche su Linux, e la cosa ha ingannato GMP
...
! **Non e' la prima volta che lo incontriamo**, e la risposta era gia'
...
! **486 e' il minimo che EX-OS gia' richiede per conto suo**,
...
! **Il file da toccare non e' lo stesso nei tre alberi**, e non e' sempre
...
! **Con base zero il descrittore e' indistinguibile da `0x23`.** I
...
## ! Il pezzo che non era ovvio: gli stub degli interrupt
...
## ! E i linker script: due righe in diciotto file
...
! **Il TLS dinamico.** `__tls_get_addr`, i modelli general-dynamic e
...
## ! Il compilatore installato non aveva le modifiche della sessione (f)
...
! Il terzo ha avuto una coda: `.eh_frame` restava anche con il
...
! **La posizione non e' condivisa, e su POSIX lo sarebbe.** L'offset sta
...
! **I tre NULL nella tabella `funcs` — pipe, fdopenr, fdopenw — non sono
...
`libiberty/configure`, `configure.ac` e `Makefile.in`. ! **Si tocca
...
| `chmod`, `fchmod`, `umask` | `bfd/opncls.c` | ! **non fanno niente** |
...
| **`mkdir` a due argomenti** | `binutils/bucomm.c` | ! cambio di firma |
| `strcoll` | `binutils/nm.c` | nella locale "C" e' `strcmp` |
| `mbrtowc`, `MB_CUR_MAX` | `binutils/readelf.c` | |
| `utime`, `<utime.h>` | `binutils/rename.c` | ! **non fa niente** |
...
! **`EOF` non c'era.** Il valore c'era dal principio — `fgetc()` ritorna
...
! **`strerror` tornava `const char *`.** Piu' sicuro, e **incompatibile**:
...
! **`mkdir` ora prende due argomenti.** Era `mkdir(const char *)` — piu'
...
! **`chmod`, `fchmod`, `umask` e `utime` non cambiano niente**, e fino a ieri non
...
## ! Due trappole della catena, che costano un'ora a testa
...
! e non basterebbe cambiare l'ordine delle librerie, perche' **`libc.a` e'
...
```
/cdrom/bin/as -o /prova.o /cdrom/prova.s      -> PROVA.O, 652 byte  ok
...
Windows — e se il sistema non ne offre **nessuna** ! **cade in fondo alla
...
simbolici da seguire (! il `..` si risolve sulla stringa, e senza
collegamenti le due cose coincidono sempre).

## ! E il nostro `<limits.h>` non lo includeva nessuno
...
! **La prova che il configure fa per il TLS e' una COMPILAZIONE, non
...
! E' vuota e non `none`, e la differenza costa una ricostruzione: con
...
! E va **esportata**, perche' deve valere anche durante il `make`: il
...
! **Su ext2 e FAT16/32 non succedeva**: li' l'offset il VFS lo passava
...
! **Resta un caso non coperto, ed e' scritto nel codice**: un BUCO — una
...
! Aggiunta una protezione a `prepara-cross.sh`, che si rilancia quasi
...
## ! L'estensione passa da una parola magica, non da due argomenti in piu'
...
! **`rename()` copia e cancella**, e sta scritto nel codice: il VFS non ha
...
## ! Due macro con il nome sbagliato, trovate dal test al primo giro
...
! **Questo spiega anche l'anomalia annotata come "osservata una volta e
...
! Il terreno e' piu' favorevole di quanto sembri: **nessun chiamante di
...
! Il DMA **non toglie il costo per richiesta**, e su ext2 con blocchi da
...
**3. ! E poi la fascia se la mangiavano le pagine utente.** Al primo giro
...
! L'eccezione dichiarata resta il caricatore dei moduli (`dynlink.c`,
...
- Regressione **ext2** su `dischi/hd.img`: `mount hd0p1 /disk`, `mkdir`,
...
! **Sul bersaglio `int32_t` e' `long int`, non `int`.** Corretto (32 bit
...
## ! I binari erano raddoppiati, e il rimedio vale per sempre
...
! Conseguenza del supporto, da tenere presente quando arrivera' il
...
## ! Il caso che ha richiesto un giro di debug: «disco inserito, non lo vede»
...
! **Niente giornale.** ext2 non ne ha e questo driver non ne inventa uno:
...
! **Unica eccezione, ed è deliberata**: se il settore 0 non ha la firma
...
! **Il prezzo, ed e' scritto in tre posti**: la mappa vale finche' quei
...
## ! La trappola piu' costosa della sessione: kernel.cfg troncato in silenzio
...
## ! VIRTUALBOX: attaccare il disco al controller IDE, non SATA
...
## ! COSA È FATTO E COSA NO — leggere prima di tutto il resto
...
! **Questo percorso è verificato solo a metà**: QEMU risponde
...
## ! Il floppy USB: un problema che nessuna correzione del driver risolve
...
### ! Trappola trovata: PowerShell, UTF-8 e i trattini lunghi
...
| ACPI vero | no | richiede un parser RSDP → RSDT → FADT → DSDT per trovare PM1a_CNT e i valori SLP_TYP. Lavoro a sé |
| APM (INT 15h) | no | funzionerebbe sull'hardware d'epoca, ma INT 15h è una chiamata in **real mode**: da protected mode servirebbe tornare in real mode o un monitor virtual-8086 |
